...
For this assignment, the first step is to load the data file gactivity.csvh by read.csv
...
Then we process/transform the data set for later analysis. In particular, the type of date column is corrected, as well as getting rid of rows containing missing values and save the subset to a new data frame gmydata2h. The original data frame is kept for later data imputation.
...
Therefore the mean is `r formatC(mean(dailysum))` steps and the median is `r formatC(mean(dailysum))` steps.
...
It turns out that the interval 835 contains maximum number of steps `r formatC(step_max)`.
...
So, the original data contains `r formatC(sum(is.na(mydata$steps)))` rows with missing data.
...
First create a new factor variable gwkh in the dataset with two levels - gweekdayh and gweekendh.

```{r echo=TRUE}
# helper function to decide if a day is a week day or not
is_weekday <- function(d) {
    wd <- weekdays(d)
    ifelse (wd == "“y—j“ú" | wd == "“ú—j“ú", "weekend", "weekday")
}
...
Next we make a panel plot containing a time series plot (i.e. type = glh) of the 5-minute interval (x-axis) and the average number of steps taken, averaged across all weekday days or weekend days (y-axis).
